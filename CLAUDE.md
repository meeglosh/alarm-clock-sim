...
- Both products currently show `state: MISSING_METADATA` — expected and harmless. Apple requires an App Store review screenshot for the *first* IAP/subscription ever submitted with an app; that only gates actual App Review submission, not sandbox/TestFlight purchases, which already work against this exact config. Not yet done (needs an image upload via a reserve-then-PUT flow); ask for it explicitly when getting close to a real submission.
