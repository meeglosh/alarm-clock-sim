...
- The clock's baked "06:30 AM" + bell LCD is covered at runtime by a live countdown/current-time panel. `SceneImageLayout` (DesignSystem.swift) maps image-fraction coordinates onto the scaledToFill render so overlays stay glued to image features across devices. The panel rect (`x: 0.2675, y: 0.480, width: 0.75, height: 0.135`, shared by MainMenuView and PlayView) is deliberately wider/taller than the visible glass so it fully hides the baked "AM" and bell icon at any rotation — narrower rects reintroduced a leak. If the background art changes, retune both the rect and `clockGlassPerspective`.
- `clockGlassPerspective()` (DesignSystem.swift) gives the LCD panel the clock's camera-angle depth: a Y-axis `rotation3DEffect` **anchored near the panel's left edge** (`UnitPoint(x: 0.12, y: 0.5)`), not center. A centered anchor makes both the left and right edges move (recede in mirror image), and at any real foreshortening strength the top-right corner visibly sags below the bezel's top edge relative to the top-left — this was flagged twice as looking wrong before landing on the left-anchored version, which keeps the already-correctly-aligned left edge fixed and only recedes the right side. Verified by cropping the baked art's own clock face and comparing pixel-for-pixel against Simulator screenshots (`sips`/PIL crops), not by eyeballing the full screenshot.
